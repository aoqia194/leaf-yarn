        ...
        MappingReader.read(path, new MappingVisitor() {
            ...
            public boolean visitField(String srcName, @Nullable String srcDesc) throws IOException {
                if (srcName.startsWith("field_")) {
                    errors.add(
                        "Encountered mapping for non-existent field " + clsSrcName + "#" + srcName + ":" + srcDesc);
                }
            ...
            public boolean visitMethodVar(int lvtRowIndex,
                ...
                int startOpIdx,
                int endOpIdx,
                @Nullable String srcName) throws IOException {
            ...
            public boolean visitElementContent(MappedElementKind targetKind) throws IOException {
                if (targetKind == MappedElementKind.METHOD) {
                    // Checking if the srcName is an intermediary name (like for classes and fields) would be the
                    // more correct option,
                    // but Enigma's bridge method mapper behaves weirdly and injects copies of the mapping into the
                    // whole hierarchy.
                    ...
                    if (mthDstNames.size() == 1 && mthDstNames.contains(mthSrcName) &&
                        mthSrcName.startsWith("method_")) {
                        errors.add("Encountered mapping for non-existent method " + clsSrcName + "#" + mthSrcName +
                                   mthSrcDesc);
                    }
                }
