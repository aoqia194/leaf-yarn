...
public final class PoetClassMethodSignatureVisitor extends SignatureVisitor {
    ...
    public PoetClassMethodSignatureVisitor(TypeAnnotationMapping mapping,
        ClassStaticContext context,
        boolean forClass) {
        super(Opcodes.ASM9);
    ...
    private void collectGeneric() {
        ...
        if (currentGenericName != null) {
            TypeVariableName generic = TypeVariableName.get(currentGenericName,
                currentGenericBounds.toArray(new TypeName[0]));
            TypeAnnotationBank bank = mapping.getBank(TypeReference.newTypeParameterReference(
                forClass ? TypeReference.CLASS_TYPE_PARAMETER : TypeReference.METHOD_TYPE_PARAMETER, generics.size()));
    ...
    public SignatureVisitor visitSuperclass() {
        ...
        return pendingSupertype = new PoetTypeSignatureWriter(mapping.getBank(TypeReference.newSuperTypeReference(-1)),
            context);
    }
    ...
    public SignatureVisitor visitInterface() {
        ...
        return pendingSupertype = new PoetTypeSignatureWriter(mapping.getBank(TypeReference.newSuperTypeReference(
            superTypes.size() - 1)), context);
    }
    ...
    public SignatureVisitor visitParameterType() {
        ...
        return pendingSlot = new PoetTypeSignatureWriter(mapping.getBank(TypeReference.newFormalParameterReference(
            params.size())), context);
    }
    ...
    public SignatureVisitor visitReturnType() {
        ...
        return pendingSlot =
            new PoetTypeSignatureWriter(mapping.getBank(TypeReference.newTypeReference(TypeReference.METHOD_RETURN)),
            context);
    }
    ...
    public SignatureVisitor visitExceptionType() {
        ...
        return pendingSlot =
            new PoetTypeSignatureWriter(mapping.getBank(TypeReference.newExceptionReference(throwables.size())),
            context);
    }
