...
package dev.aoqia.leaf.filament.mappingpoet;
...
public class MethodBuilder {
    ...
    public MethodBuilder(MappingsStore mappings,
        ClassNode classNode,
        MethodNode methodNode,
        Environment environment,
        String receiverSignature,
        int formalParamStartIndex) {
    ...
    private MethodSpec.Builder createBuilder() {
        ...
        if (methodNode.name.equals("<init>") || !java.lang.reflect.Modifier.isInterface(classNode.access) ||
            java.lang.reflect.Modifier.isPrivate(methodNode.access)) {
            builder.modifiers.remove(Modifier.DEFAULT);
        }

        if (methodNode.signature != null) {
            signature = AnnotationAwareSignatures.parseMethodSignature(methodNode.signature,
                typeAnnotations,
                environment);
    ...
    private void setReturnType() {
        ...
        if (signature != null) {
            ...
            typeName = AnnotationAwareDescriptors.parseDesc(returnDesc,
                typeAnnotations.getBank(TypeReference.newTypeReference(TypeReference.METHOD_RETURN)),
                environment);
        }
    ...
    private void addParameters(MethodBuilder this) {
        ...
        TypeAnnotationBank receiverAnnos =
            typeAnnotations.getBank(TypeReference.newTypeReference(TypeReference.METHOD_RECEIVER));
        ...
        if (!receiverAnnos.isEmpty()) {
            ...
            if (methodNode.name.equals("<init>")) {
                TypeName annotatedReceiver = AnnotationAwareSignatures.parseSignature(
                    "L" + receiverSignature.substring(0, receiverSignature.lastIndexOf('.')) + ";",
                    receiverAnnos,
                    environment);
                ...
                TypeName annotatedReceiver = AnnotationAwareSignatures.parseSignature("L" + receiverSignature + ";",
                    receiverAnnos,
                    environment);
        ...
        for (ParamType paramType : paramTypes) {
            ...
            ParameterSpec.Builder paramBuilder = ParameterSpec.builder(paramType.type,
                paramType.name,
                paramType.modifiers);
    ...
    private void getParams(List<ParamType> paramTypes, boolean instance, Set<String> usedParamNames) {
        ...
        Iterator<TypeName> signatureParamIterator =
            signature == null ? Collections.emptyIterator() : signature.parameters().iterator();
        ...
        while (desc.charAt(index) != ')') {
            ...
            if (paramIndex >= formalParamStartIndex) { // skip guessed synthetic/implicit params
                ...
                if (signatureParamIterator.hasNext()) {
                    ...
                    parsedType = AnnotationAwareDescriptors.parseDesc(desc.substring(oldIndex, index),
                        typeAnnotations.getBank(TypeReference.newFormalParameterReference(
                            paramIndex - formalParamStartIndex)),
                        environment);
                }

                paramTypes.add(new ParamType(mappings.getParamNameAndDoc(environment,
                    classNode.name,
                    methodNode.name,
                    methodNode.desc,
                    slot), parsedType, usedParamNames, slot));
            }
        ...
        ParamType(Map.Entry<String, String> nameAndDoc, TypeName type, Set<String> usedNames, int slot) {
            ...
            if (this.name != null) {
                if (usedNames.contains(this.name)) {
                    System.err.printf("Overridden parameter name detected in %s %s %s slot %d, resetting%n",
                        classNode.name,
                        methodNode.name,
                        methodNode.desc,
                        slot);
