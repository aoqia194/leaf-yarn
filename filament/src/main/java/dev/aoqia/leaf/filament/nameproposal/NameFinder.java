            ...
            for (MappingTree.ClassMapping classMapping : mappingTree.getClasses()) {
                for (MappingTree.FieldMapping fieldMapping : classMapping.getFields()) {
                    if (fieldMapping.getName(intermediaryId).equals(entry.getKey())) {
                        MappingEntry fieldEntry = new MappingEntry(classMapping.getName(intermediaryId),
                            fieldMapping.getName(intermediaryId),
                            fieldMapping.getDesc(intermediaryId));
                ...
                for (MappingTree.MethodMapping methodMapping : classMapping.getMethods()) {
                    if (methodMapping.getName(intermediaryId).equals(entry.getKey())) {
                        MappingEntry fieldEntry = new MappingEntry(classMapping.getName(intermediaryId),
                            methodMapping.getName(intermediaryId),
                            methodMapping.getDesc(intermediaryId));
