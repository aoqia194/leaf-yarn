...
package dev.aoqia.leaf.filament.nameproposal;
...
public class FieldNameFinder {
    ...
    public Map<MappingEntry, String> findNames(Map<String, Set<String>> allEnumFields,
        Map<String, List<MethodNode>> classes) {
            ...
            for (MethodNode mn : entry.getValue()) {
                ...
                for (int i = 1; i < instrs.size(); i++) {
                    ...
                    if (instr2.getOpcode() == Opcodes.PUTSTATIC && ((FieldInsnNode) instr2).owner.equals(owner)
                        ...
                        && (instr1.getOpcode() == Opcodes.INVOKESTATIC ||
                    ...
                    if (s != null) {
                        ...
                        for (int j = 0; j < s.length(); j++) {
                            ...
                            } else if (j > 0 && Character.isUpperCase(s.charAt(j)) &&
                                       Character.isLowerCase(s.charAt(j - 1))) {
                                s = s.substring(0, j) + "_" + s.substring(j, j + 1).toLowerCase(Locale.ROOT) +
                                    s.substring(j + 1);
                            }
                        }
                        ...
                        if (hasAlpha) {
                            ...
                            Set<String> usedNames = fieldNamesUsed.computeIfAbsent(((FieldInsnNode) instr2).owner,
                                (a) -> new HashSet<>());
                            Set<String> usedNamesDuplicate =
                                fieldNamesDuplicate.computeIfAbsent(((FieldInsnNode) instr2).owner,
                                (a) -> new HashSet<>());
                            ...
                            if (usedNames.contains(s)) {
                                fieldNames.put(new MappingEntry(((FieldInsnNode) instr2).owner,
                                    ((FieldInsnNode) instr2).name,
                                    ((FieldInsnNode) instr2).desc), s);
                            }
                        }
                    }
                }
