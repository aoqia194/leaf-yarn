...
public final class AnnotationAwareDescriptors {
    ...
    public static ClassSignature parse(String rawSuper,
        List<String> rawInterfaces,
        TypeAnnotationMapping mapping,
    ...
    /**
     * Annotate class name chains until the last element. Useful for making the last element a parameterized type before
     * annotating it.
     *
     ...
     */
    static Map.Entry<ClassName, TypeAnnotationBank> annotateUpTo(String internalName,
        TypeAnnotationBank bank,
        ClassStaticContext context) {
        ...
        if (moneySign == -1) {
            return new AbstractMap.SimpleImmutableEntry<>(ClassName.get(packageSt, internalName.substring(slice + 1)),
                bank);
        }
