...
import dev.aoqia.leaf.filament.mappingpoet.Environment.ClassNamePointer;
import dev.aoqia.leaf.filament.mappingpoet.Environment.NestedClassInfo;
...
public class MappingPoet {
    public static void generate(Path mappings, Path inputJar, Path outputDirectory, List<Path> libraries) {
        ...
        forEachClass(inputJar,
            (classNode, environment) -> writeClass(mapping, classNode, classes, environment),
            libraries);
        ...
        for (ClassBuilder classBuilder : classes.values()) {
            ...
            try {
                ...
                String pkgName =
                    packageEnd < 0 ? "" : classBuilder.getClassName().substring(0, packageEnd).replaceAll("/", ".");
                    ...
                    if (classNode.innerClasses != null) {
                        for (InnerClassNode e : classNode.innerClasses) {
                            if (e.outerName != null) {
                                ...
                                nestedClasses.put(e.name,
                                    new NestedClassInfo(e.outerName, !Modifier.isStatic(e.access), e.innerName));
                            }
                        }
                    }
    ...
    private static void scanNestedClasses(Map<String, ClassNamePointer> classNames,
        Map<String, Environment.NestedClassInfo> instanceInnerClasses,
        List<Path> libraries) {
        try {
                        ...
                        try (InputStream is = jarFile.getInputStream(entry)) {
                            ...
                            reader.accept(new ClassVisitor(Opcodes.ASM9) {
                                ...
                                public void visitInnerClass(String name,
                                    ...
                                    String simpleName,
                                    int access) {
                                    instanceInnerClasses.put(name,
                                        new NestedClassInfo(outerName, !Modifier.isStatic(access), simpleName));
