...
package dev.aoqia.leaf.filament.nameproposal.enigma;
...
import dev.aoqia.leaf.filament.nameproposal.MappingEntry;
import dev.aoqia.leaf.filament.nameproposal.NameFinder;
...
public class EnigmaNameProposalService implements JarIndexerService, NameProposalService {
    ...
    public Optional<String> proposeName(Entry<?> obfEntry, EntryRemapper remapper) {
        ...
        if (obfEntry instanceof FieldEntry fieldEntry) {
            ...
            return Optional.ofNullable(fieldNames.get(new MappingEntry(fieldEntry.getContainingClass().getFullName(),
                fieldEntry.getName(),
                fieldEntry.getDesc().toString())));
