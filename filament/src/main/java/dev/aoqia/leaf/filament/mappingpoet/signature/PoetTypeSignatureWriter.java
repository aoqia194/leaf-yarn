 ...
 * It does not include class or method signatures where new generics like {@code <T>} can be defined.</p>
 */
public final class PoetTypeSignatureWriter extends SignatureVisitor {
    ...
    public void visitClassType(String internalName) {
        Map.Entry<ClassName, TypeAnnotationBank> entry = AnnotationAwareDescriptors.annotateUpTo(internalName,
            storage,
            context);
        ...
        if (!params.isEmpty()) {
            if (currentType instanceof ParameterizedTypeName) {
                ...
                if (nestedClassName == null) { // top-level
                    ...
                    currentType = ParameterizedTypeName.get(((ClassName) currentType).nestedClass(nestedClassName),
                        params.toArray(new TypeName[0]));
                }
            }
    ...
    private void collectLastTypeArgument() {
        if (activeTypeArgument != null) {
            ...
            TypeName used = switch (activeTypeArgumentKind) {
                ...
                default -> throw new IllegalStateException(String.format("Illegal type argument wildcard %s",
                    activeTypeArgumentKind));
            };
