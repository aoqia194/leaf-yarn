...
import dev.aoqia.leaf.filament.util.FileUtil;
import dev.aoqia.leaf.filament.util.UnpickUtil;
    ...
    public abstract static class RemapAction implements WorkAction<RemapParameters> {
        ...
        public void execute() {
            try {
                ...
                for (MappingTree.ClassMapping classDef : mappingTree.getClasses()) {
                    ...
                    for (MappingTree.MethodMapping methodDef : classDef.getMethods()) {
                        methodMappings.put(
                            new MethodKey(
                                ...
                                Objects.requireNonNull(methodDef.getName(fromM),
                                    "Null dst name: " + methodDef.getSrcName()),
                                Objects.requireNonNull(methodDef.getDesc(fromM),
                                    "Null dst name: " + methodDef.getSrcName())
                            ),
                    ...
                    for (MappingTree.FieldMapping fieldDef : classDef.getFields()) {
                        fieldMappings.put(
                            new FieldKey(
                                Objects.requireNonNull(classFromName, "Null dst name: " + classDef.getSrcName()),
                                Objects.requireNonNull(fieldDef.getName(fromM),
                                    "Null dst name: " + fieldDef.getSrcName())
                            ),
                ...
                try (UnpickV2Reader reader = new UnpickV2Reader(new FileInputStream(getParameters().getInput()
                    .getAsFile()
                    .get()))) {
