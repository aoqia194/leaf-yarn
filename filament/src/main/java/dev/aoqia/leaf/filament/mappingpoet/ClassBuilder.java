...
import static dev.aoqia.leaf.filament.mappingpoet.FieldBuilder.parseAnnotation;
...
public class ClassBuilder {
    static final Handle OBJ_MTH_BOOTSTRAP = new Handle(
        ...
        "bootstrap",
        "(Ljava/lang/invoke/MethodHandles$Lookup;Ljava/lang/String;Ljava/lang/invoke/TypeDescriptor;Ljava/lang/Class;" +
    ...
    private ClassSignature setupSignature() {
        if (classNode.signature == null) {
            return AnnotationAwareDescriptors.parse(classNode.superName,
    ...
    private TypeSpec.Builder setupBuilder() {
        ...
        if (Modifier.isInterface(classNode.access)) {
            if (classNode.interfaces.size() == 1 &&
                classNode.interfaces.get(0).equals("java/lang/annotation/Annotation")) {
        ...
        do {
            ...
            if (ch == '$' || ch == ';') {
                ...
                if (currentClassName == null) {
                    String packageName =
                        nameStart < classNameSeparator ? internalName.substring(nameStart, classNameSeparator)
                            .replace('/', '.') : "";
...
methodsLoop:
        for (MethodNode method : classNode.methods) {
            ...
            if (enumClass) {
                ...
                if (method.name.equals("valueOf") &&
                    method.desc.equals("(Ljava/lang/String;)L" + classNode.name + ";")) {
                    continue;
                }
            ...
            builder.addMethod(new MethodBuilder(mappings,
                classNode,
                method,
                environment,
                receiverSignature,
                formalParamStartIndex).build());
        }
    }
    ...
    private void addFields() {
        ...
        for (FieldNode field : classNode.fields) {
            if (recordClass && !Modifier.isStatic(field.access)) {
                ...
                if (!Modifier.isFinal(field.access) || Modifier.isProtected(field.access) ||
                    Modifier.isPublic(field.access)) {
                    System.out.println(
                        "abnormal instance field " + field.name + " in record " + getClassName() + ", skipping");
                } else {
    ...
    public void addInnerClass(ClassBuilder classBuilder) {
        ...
        if (innerClassNode == null) {
            ...
            classBuilder.builder.modifiers.remove(javax.lang.model.element.Modifier.PUBLIC); // this modifier may
            // come from class access
