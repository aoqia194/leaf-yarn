...
public class MappingsStore {
    ...
    public void addFieldDoc(DocAdder addJavadoc, String owner, String name, String desc) {
        ...
        for (int id = SRC_NAMESPACE_ID; id < maxNamespace; id++) {
            ...
            addJavadoc.addJavadoc("@mapping {@literal $L:$L:$L}\n",
                tree.getNamespaceName(id),
                transformedName,
                mixinForm);
        }
    }

    public Map.Entry<String, String> getParamNameAndDoc(Environment environment,
        String owner,
        String name,
        String desc,
        int index) {
    ...
    private Map.Entry<ClassMappingView, MethodMappingView> searchMethod(Environment environment,
        String owner,
        String name,
        String desc) {
        var classDef = tree.getClass(owner);
