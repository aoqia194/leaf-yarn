...
import dev.aoqia.leaf.filament.mappingpoet.FieldBuilder;
...
public final class TypeAnnotationStorage implements TypeAnnotationMapping, TypeAnnotationBank {
    ...
    TypeAnnotationStorage(int startIndex,
        int endIndex,
        String currentPath,
        int[] targets,
        String[] paths,
        AnnotationSpec[] contents) {
    ...
    public TypeAnnotationBank advance(int step, int stepArgument) {
        ...
        String hiCheck = check.substring(0, check.length() - 1)
            .concat(Character.toString((char) (check.charAt(check.length() - 1) + 1)));
    ...
    public static final class Builder {
        ...
        public Builder add(Iterable<TypeAnnotationNode> nodes) {
            ...
            for (TypeAnnotationNode node : nodes) {
                entries.add(new Entry(node.typeRef,
                    node.typePath == null ? "" : node.typePath.toString(),
                    FieldBuilder.parseAnnotation(node)));
            }
